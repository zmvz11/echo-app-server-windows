...
export type EchoApp = { id: string; name: string; shortDescription: string; fullDescription: string; developer: string; category: string; tags: string[]; platforms?: string[]; visibility: AppVisibility; featured?: boolean; media: AppMedia[]; createdAt: string; updatedAt: string; };
export type AppRelease = { id: string; appId: string; version: string; channel: ReleaseChannel; status: ReleaseStatus; platform: PlatformId; packageUrl: string; packageFileName?: string; sizeBytes?: number; entrypoint: string; installType: 'portable' | 'installer'; changelog: string[]; releaseNotes?: string; createdAt: string; submittedAt?: string; approvedAt?: string; approvedBy?: string; rejectedAt?: string; rejectedBy?: string; rejectReason?: string; publishedAt?: string; rolledBackAt?: string; };
...
export type Database = { users: User[]; sessions: Session[]; apps: EchoApp[]; releases: AppRelease[]; clients: ClientDevice[]; installReports: InstallReport[]; auditLogs: AuditLog[]; };
